...
# CODE GOES HERE
    def cbState(self, msg):
        ...
        self.state = True
        rospy.loginfo('We reached now %s stripes, out of XX' %msg.data)
                    ...
                    if self.plan == True:
                        ...
                        if trigger == True: # detected AT is in list, use as starting point
                            ...
                            if len(self.path) > 1: #can be 1, since last AT is omitted from self.path
                                ...
                                turn_cmd = self.pathProcessor(self.path, self.cmd)
                                self.publishCmd(turn_cmd)
                                rospy.loginfo('Continue to lastMile module')
    ...
    def lastMile(self):
        # From kinematics_node
        self.speed = self.gain
        # adjusted
        self.speed_adj

        self.time = self.goal_distance / self.speed_adj
        ...
        if self.fsm_mode != "INTERSECTION_CONTROL" and self.fsm_mode != "INTERSECTION_COORDINATION" and self.fsm_mode != "INTERSECTION_PLANNING":
            if rospy.time = self.time:
                self.publishStop()
                rospy.loginfo('We reached our final objective!')
                self.onShutdown()
            else:
                pass
        else:
